...
type Database struct {
	diskdb xordb.KeyValueStore // Persistent storage for matured trie nodes
...
func NewDatabase(diskdb xordb.KeyValueStore) *Database {
...
func NewDatabaseWithCache(diskdb xordb.KeyValueStore, cache int) *Database {
...
func (db *Database) DiskDB() xordb.Reader {
	return db.diskdb
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			...
			if batch.ValueSize() > xordb.IdealBatchSize {
...
func (db *Database) commit(hash common.Hash, batch xordb.Batch, uncacher *cleaner) error {
