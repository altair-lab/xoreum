...
func main() {
	...
	if last_BN == nil {
		...
		// Connect to ready port
		_, err := net.Dial("tcp", host+":"+readyport)
		if nil != err {
			log.Fatal("failed to connect to ready server")
		}
		log.Println("Connected to ready server!")
	} else {
		...
		for {
			...
			if nil != err {
				if io.EOF == err {
					...
					os.Exit(1) // [For evaluation] After synchronization, terminate the full node
					quit <- true
					return
				}
